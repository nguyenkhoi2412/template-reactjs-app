...
export const QUESTION_DELETE = createAsyncThunk(
  "question/delete",
  async (params, thunkAPI) => {
    return await apiServices.delete(`question/delete/`, params);
  }
);
...
export const question = createSlice({
  ...
  extraReducers: {
    ...
    //#endregion
    //#region QUESTION_DELETE
    [QUESTION_DELETE.pending]: (state) => {
      return {
        ...state,
        isFetching: true,
      };
    },
    [QUESTION_DELETE.rejected]: (state) => {
      return {
        ...state,
        isFetching: false,
      };
    },
    [QUESTION_DELETE.fulfilled]: (state, action) => {
      const payload = action.payload;
      const currentState = current(state);
